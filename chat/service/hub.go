...
func (hub *Hub) ServeWs(ctx *gin.Context) {
	...
	if err != nil {
		if strings.Contains(err.Error(), postgres.ErrSqlNoRows) {
			rest_api_utils.BindNoContent(ctx)
			return
		}
	...
	clientConnection := newConnection(ws, user, roomName)
	...
	// отправляем все сообщения из лога.
	for i := range currentRoom.messages {
		// json.
		marshalledMessage, err := jsoniter.Marshal(currentRoom.messages[i])
		// proto.
		//marshalledMessage, err := currentRoom.messages[i].Marshal()
		if err != nil {
			log.Println(err.Error())
		}
		clientConnection.send <- marshalledMessage
	}
}
